#!/usr/bin/env python
"""Redis cache and lock performance tests"""
...
def test_cache_hit_ratio():
    ...
    print(f"\n1⃣ First request with key: {idempotency_key[:8]}...")
    start = time.time()
    ...
    print(f"\n2⃣ Duplicate request (should be CACHE HIT)")
    start = time.time()
...
def test_concurrent_cache():
    ...
    print(f"\n1⃣ Warming up cache with key: {idempotency_key[:8]}...")
    requests.post(
        f"{BASE_URL}/api/v1/payments",
        headers={"Idempotency-Key": idempotency_key},
        json={"amount": 50, "currency": "USD", "source": "card_123"}
    ...
    print(f"\n2⃣ Sending 50 concurrent duplicate requests...")
    start = time.time()
...
def test_distributed_lock():
    ...
    print(f"     Conflicts (lock contention): {conflicts}")
